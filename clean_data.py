...
def create_data():
    ...
    common_data['Name'] = structure_lot
    ...
    # add a column of size
    for df in day_dataframes:
        sizes = np.array([])
        for i in np.arange(16):
            if df['Total Spaces'][i] < 230:
                sizes = np.append(sizes,10)
            elif df['Total Spaces'][i] > 230 and df['Total Spaces'][i] < 460:
                sizes = np.append(sizes,20)
            elif df['Total Spaces'][i] > 460 and df['Total Spaces'][i] < 690:
